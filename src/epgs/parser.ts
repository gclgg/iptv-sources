...
function parseXmltvTime(startStr: string, stopStr: string): { date: string; start: string; end: string } | null {
...
function pickTitle(programme: Record<string, unknown>): string {
    ...
    if (Array.isArray(title)) {
        const first = title[0]
        return typeof first === "string" ? first.trim() : (first as { "#text"?: string })?.["#text"]?.trim() ?? ""
    }
 ...
 */
export function parseEpgXml(xml: string): Array<{ date: string; channel: string; item: EpgProgrammeItem }> {
